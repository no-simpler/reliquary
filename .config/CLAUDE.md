...
The build cache is held down by `[profile.dev] debug = "line-tables-only"` and by `up`'s **Cargo build cache** step, above a 2 GiB per-lane ceiling: `cargo sweep --installed`, then `--time 30`, then **`rm -rf target/*/incremental`**. That last one is the only part that acts on a workspace built every day — `cargo sweep` reasons about build *artifacts* and no flag of its reaches incremental state, which grew to 6.2 GiB in 707 session directories because `fmt`, `clippy`, `nextest`, `llvm-cov` and `mutants` each compile under their own flags and cargo garbage-collects within a fingerprint family, never across. Incremental state is a pure speed cache, so removing it costs the next build's speed and nothing else; steady state after a full verify run is ~1 GiB. Skipped silently under the ceiling or when `cargo-sweep` is absent. `yadm doctor` reports the size.
