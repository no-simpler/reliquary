...
printf '[relic]\nname = "pub"\nruntime = "bash"\n' >"$RELICS_LANE/pub/relic.toml"
printf '[relic]\nname = "secret"\nruntime = "bash"\n' >"$ATTIC_LANE/secret/relic.toml"
chmod 000 "$ATTIC_LANE/secret/relic.toml" # simulate undecrypted/unreadable
...
chmod 644 "$ATTIC_LANE/secret/relic.toml"
listing="$(inhouse_relics)"
...
check "doctor_unpublished flags extra" "$(inhouse_relics | doctor_unpublished | awk -F'\t' '$2=="extra"{print $1}')" "tool"
check "doctor_unpublished skips tool" "$(inhouse_relics | doctor_unpublished | awk -F'\t' '$2=="tool"{print "y"}')" ""
